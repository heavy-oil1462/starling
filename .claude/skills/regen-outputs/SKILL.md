...
- After running, read the output: every part must be `[ok]` and `volumes=1`. A second volume means coincident faces in a `difference()` — fix the cutter, don't ship the STL.
